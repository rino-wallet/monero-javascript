...
class MoneroWalletFull extends MoneroWalletKeys {
  ...
  async exportOutputs(all, complete) {
    ...
    return that._module.queueTask(async function() {
      that._assertNotClosed();
      return new Promise(function(resolve, reject) {
        that._module.export_outputs(that._cppAddress, all, complete, function(outputsHex) { resolve(outputsHex); });
      });
    });
  }
  ...
  static async verifyMessageStatic(message, address, signature, networkType) {
    ...
    return module.queueTask(async function() {
      let resultJson;
      try {
        resultJson = JSON.parse(module.verify_message_static(message, address, signature, networkType));
      } catch (err) {
        resultJson = {isGood: false};
      }
      let result = new MoneroMessageSignatureResult(
...
class MoneroWalletFullProxy extends MoneroWallet {
  ...
  async exportOutputs(all, complete) {
    return this._invokeWorker("exportOutputs", [all, complete]);
  }
  ...
  async reconstructValidateTx(multisigTxHex, config) {
    //Normalise config and pass as json to worker (which can re-normalize it).
    config = MoneroWallet._normalizeCreateTxsConfig(config);
    let txSetJson = await this._invokeWorker("reconstructValidateTx", [multisigTxHex, config.toJson()]);
    return new MoneroTxSet(txSetJson).getTxs();
  }
